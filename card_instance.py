...
def read_landing_table_for_card_instance_data():
    try:
        ...
        query = f"""SELECT lpc.row_id,
                    ...
                    JOIN {DB_FACTS_SCHEMA}.{DB_CARD_TABLE} cs ON lpc.card = cs.card
...
def insert_card_instance_data():
    try:
        ...
        if df is not None:
            ...
            df.to_sql(DB_CARD_INSTANCE_TABLE, con=ENGINE, schema=DB_FACTS_SCHEMA, if_exists='append', index=False)
